...
| 2 | C++, libpcap | Simple program that takes a pcap file as input parameter. A the descriptor is obtained with the `pcap_open_offline()`. Once it has been correctly opened a packet will be captured with the `pcap_next()` and some information on its content will be printed out. VLAN traffic ([802.1Q](https://en.wikipedia.org/wiki/IEEE_802.1Q)) has been managed. |